...
class DataGenerator:
    def __init__(self, filelist, data_shape=np.array([29, 129]), seq_len = 3, shuffle=False, test_mode=False):
    ...
    def read_mat_filelist(self,filelist):
        ...
        for i in range(len(files)):
            ...
            self.boundary_index = np.append(self.boundary_index, np.arange(count, count + np.floor(self.seq_len/2)))
            self.boundary_index = np.append(self.boundary_index, np.arange(count + len(X) - np.floor(self.seq_len/2), count + len(X)))
            count += len(X)
            print(count)
    ...
    def next_batch(self, batch_size):
        ...
        for i in range(len(data_index)):
            batch_x[i]  = self.X[int(data_index[i]), :, :, :]
            for n in range(self.seq_len):
                batch_y[i, n] = self.y[int(data_index[i] - np.floor(self.seq_len/2) + n), :]
                batch_label[i, n] = self.label[int(data_index[i] - np.floor(self.seq_len/2) + n)]
